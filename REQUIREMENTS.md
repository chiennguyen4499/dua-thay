...
- Tab **Dự đoán**: nhập tay theo đúng cấu trúc game — **2 yêu quái bội THẤP (3–5) + 2 yêu quái bội CAO (6–12)** + bội Thầy (`Duong_tang` cố định, không cần chọn tên). Cả **tên lẫn bội đều chọn bằng nút 1 chạm** (không dropdown) — mỗi nhóm chỉ gợi ý đúng các con thuộc dải bội đó (9 con bội thấp/cao xếp ABC). Con đã chọn ở slot cùng nhóm bị **làm mờ (disable)** thay vì ẩn đi → không nhảy layout. Luật game cho **cược tối đa 2 con/trận** → kết quả dẫn đầu bằng **cặp kèo nên đánh (tối đa 2 con)**, chỉ gồm con nằm trên bội đã chứng minh có lãi (thực tế: bội 5 & 9), kèm **mức cược gợi ý (¼-Kelly)**; trận không có con đủ tin thì khuyên **bỏ trận** (không cược ép). Có nút **Nhập trận mới**. Đầu tab có ghi nhớ **kèo heuristic theo bội tự tính từ data sống** (ROI + CI, tự cập nhật theo số trận).
