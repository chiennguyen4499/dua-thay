...
- **Dự đoán**: toàn bộ khối chọn quái + bội + nút Dự đoán gói trong **`@st.fragment _prediction_input()`** — chạm chọn tướng/bội **chỉ rerun riêng fragment**, KHÔNG render lại cả tab (header, memo heuristic, radio tab, sidebar) → hết cảm giác "load" khi chọn (dữ liệu vốn đã cache nên không phải round-trip, cái tốn là re-render + đẩy cây widget qua mạng mỗi rerun). Bấm **Dự đoán** mới `st.rerun(scope="app")` để render kết quả nằm NGOÀI fragment. Nhập theo cấu trúc game — **2 con bội THẤP (3–5) + 2 con bội CAO (6–12)**, mỗi nhóm là một khối `_monster_group(title, hint, options, group_key, boi_list, default_boi)` tối ưu cho mobile: liệt kê đủ **9 con/nhóm, 3/hàng** (`LOW_MONSTERS`/`HIGH_MONSTERS` xếp ABC), **chạm để chọn** tối đa 2 con (callback `_pick_monster`). Mỗi nhóm bọc trong `st.container(key="mgrid_<group>")` + CSS scope theo class `st-key-mgrid_*` ép `flex-wrap:nowrap` cho `stHorizontalBlock` (mặc định `st.columns` tự stack 1 cột/hàng khi màn hẹp → mobile bị 1 con/hàng); nhờ đó **giữ 3 con/hàng và hàng bội `tên·bội·✕` nằm trên 1 dòng** kể cả 7 bội 6–12 (thu nhỏ font/padding segmented). Đã đo DOM ở viewport 375px: lưới 3 cột side-by-side, button-group bội cao 32px (1 dòng), rồi hiện **hàng ▸ chọn/sửa BỘI** ngay dưới từng con đã chọn (`st.segmented_control` + nút ✕ `_remove_monster`). Chạm lại con đã chọn KHÔNG bỏ chọn (theo UX yêu cầu) — sửa bội ở hàng ▸, bỏ bằng ✕. Nhóm đủ 2 con → các con còn lại `disabled` (mờ, **không nhảy layout** vì lưới 9 nút luôn cố định). Chỉ **18 nút** thay vì 36 như kiểu 4-slot cũ (`_name_selector` đã bỏ). Trạng thái: `st.session_state["sel_low"]`/`["sel_high"]` = `{tên: bội}`; nhãn nút đọc thẳng `session_state["sel_*_boi__<tên>"]` để không trễ 1 nhịp khi đổi bội (lưới render trước hàng bội). **Bội Thầy** segmented_control (14–26), tên Thầy cố định `Duong_tang`. Nút **🆕 Nhập trận mới** (`key="btn_new_round"`, **`on_click=_reset_selection`** — callback chạy trước render nên xoá sạch, không phụ thuộc `st.rerun()`; nút CÓ key rõ để test click đúng). Nút **🔮 Dự đoán** `disabled` tới khi đủ **2 + 2** (`ready`). Kết quả render qua `render_prediction(state)`, lưu ở `st.session_state["pred"]`. Dẫn đầu bằng **KHUYẾN NGHỊ kỷ luật bội 5&9** (`bet_pair`, ô success) + **tham khảo mô hình** (`model_pair`, expander) — xem mục 4. **Nút ghi người thắng ngay dưới kết quả**. **Chống bấm trùng tạo rác** qua `sig`. Đầu tab có expander **"📌 Ghi nhớ: kèo heuristic theo bội"** (`compute_heuristic_summary()`, ROI + bootstrap CI mỗi bội).
