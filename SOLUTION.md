...
- **Soi cầu**: thống kê kiểu xổ số cho yêu quái **bội cao (≥9)**. Dùng `db.get_high_odds_appearances(9)` — 1 query UNION 4 slot trả **mọi lần ra sân ở bội≥9** kèm `won` + `round_id`, `ORDER BY round_id` (thứ tự thời gian), bọc `@st.cache_data` `_cached_high_odds_appearances` (đã thêm vào `_bust_data_cache`). Radio **phạm vi bội** (Gộp 9–12 / Chỉ 9 / Chỉ 10–12) chỉ **lọc** list này ở Python — KHÔNG tách bảng theo tên×bội (255 trận ÷ 36 ô quá thưa để "chu kỳ về" có nghĩa; luôn gom **theo tên**). Với mỗi tên (đủ `min_app` lần xuất hiện): **Đang khan** = số lần ra sân SAU lần về gần nhất (chưa về lại), **Chu kỳ TB** = Xuất hiện ÷ Về (trung bình bao nhiêu lần thì về 1), **Lần về gần nhất** (ngày). Trạng thái 🔥 quá hạn (khan ≥ chu kỳ) / 🟡 tới hạn (≥70%) / ⚪ bình thường / ❓ chưa từng về. Bảng sort **khan giảm dần** (lâu chưa về nhất lên đầu), có `ProgressColumn` cho tỉ lệ về. Có cảnh báo rõ đây là **gambler's fallacy** (game ngẫu nhiên, "quá hạn" không đảm bảo về) — chỉ tham khảo.
