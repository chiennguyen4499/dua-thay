...
- **HAI lựa chọn cược song song** (luật game: **2 cược/trận**; **cược Thầy thì chỉ 1 mình Thầy, không kèm yêu quái**):
  - **`model_pair` — ① Theo mô hình**: 2 EV cao nhất (KHÔNG lọc). Tôn trọng luật Thầy: nếu con EV cao nhất là Thầy → gợi ý **Thầy đơn** (`model_solo_teacher=True`); ngược lại lấy **2 yêu quái EV cao nhất** (bỏ Thầy). Có thể gợi ý bội cao (variance lớn) — người dùng tự cân với ②.
  - **`bet_pair` — ② Kỷ luật (bội 5 & 9)**: qua **cổng tin cậy** — chỉ nhận nhân vật có GIÁ TRỊ BỘI đã chứng minh +EV chắc: cận dưới CI Wilson của win-rate tầng bội × bội **> 1.05** và tầng ≥10 mẫu (data hiện tại chỉ **bội 5 & 9**; Thầy + bội 10/11/12 + bội 4 bị loại). Ưu tiên **favorite (bội thấp trước)**, tối đa 2 (cược 2 favorite thắng ~72%, variance thấp). Rỗng → **bỏ trận**.
  - Mỗi con ở cả 2 lựa chọn kèm `stake_fraction` (¼-Kelly, trần 5%).
