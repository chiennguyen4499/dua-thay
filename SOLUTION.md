...
- **Web UI lên Streamlit Cloud + database dùng chung online (2026-07)**: bot Telegram tiếp tục chạy trên PC, Web UI chuyển sang Streamlit Community Cloud (không có ổ đĩa bền) để dùng được từ điện thoại và vẫn ghi được kết quả từ xa. Chọn **Turso (libSQL)** thay vì Supabase/Postgres vì cùng cú pháp SQLite (không phải viết lại câu SQL), và free tier không tự "ngủ"/pause sau vài ngày không dùng (Supabase free tier pause sau 7 ngày — rủi ro thật với app ít traffic).
  ...
  - **Đã sửa — cả 5 tab đọc lại DB trên MỌI tương tác**: Streamlit chạy lại toàn bộ script (kể cả nội dung các tab không hiển thị) mỗi khi người dùng tương tác với BẤT KỲ widget nào (vd. chỉ chọn 1 yêu quái ở tab Dự đoán) — nên trước đây mỗi lần đó bắn lại ~12 round-trip Turso rải rác ở sidebar + cả 5 tab (`get_overall_stats`, `get_all_competitor_stats`, `get_odds_calibration_data`, `get_recent_rounds` gọi 2 lần khác limit, `strategy_analysis.load_rounds`...). Đã bọc các hàm đọc này bằng `st.cache_data` (không TTL) trong `web_app.py`, và gọi `_bust_data_cache()` ngay sau mọi thao tác ghi/xóa (`update_winner`, `save_round` khi tạo trận mới, `delete_pending_rounds`, import CSV) để đảm bảo luôn đúng dữ liệu mới nhất.
