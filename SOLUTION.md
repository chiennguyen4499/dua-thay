...
- **Dự đoán**: nhập theo cấu trúc game — **2 con bội THẤP (3–5) + 2 con bội CAO (6–12)**, mỗi nhóm là một khối `_monster_group(title, hint, options, group_key, boi_list, default_boi)` tối ưu cho mobile: liệt kê đủ **9 con/nhóm, 3/hàng** (`LOW_MONSTERS`/`HIGH_MONSTERS` xếp ABC), **chạm để chọn** tối đa 2 con (callback `_pick_monster`), rồi hiện **hàng ▸ chọn/sửa BỘI** ngay dưới từng con đã chọn (`st.segmented_control` + nút ✕ `_remove_monster`). Chạm lại con đã chọn KHÔNG bỏ chọn (theo UX yêu cầu) — sửa bội ở hàng ▸, bỏ bằng ✕. Nhóm đủ 2 con → các con còn lại `disabled` (mờ, **không nhảy layout** vì lưới 9 nút luôn cố định). Chỉ **18 nút** thay vì 36 như kiểu 4-slot cũ (`_name_selector` đã bỏ). Trạng thái: `st.session_state["sel_low"]`/`["sel_high"]` = `{tên: bội}`; nhãn nút đọc thẳng `session_state["sel_*_boi__<tên>"]` để không trễ 1 nhịp khi đổi bội (lưới render trước hàng bội). **Bội Thầy** segmented_control (14–26), tên Thầy cố định `Duong_tang`. Nút **🆕 Nhập trận mới** (`key="btn_new_round"`, **`on_click=_reset_selection`** — callback chạy trước render nên xoá sạch, không phụ thuộc `st.rerun()`; nút CÓ key rõ để test click đúng). Nút **🔮 Dự đoán** `disabled` tới khi đủ **2 + 2** (`ready`). Kết quả render qua `render_prediction(state)`, lưu ở `st.session_state["pred"]`. Dẫn đầu bằng **KHUYẾN NGHỊ kỷ luật bội 5&9** (`bet_pair`, ô success) + **tham khảo mô hình** (`model_pair`, expander) — xem mục 4. **Nút ghi người thắng ngay dưới kết quả**. **Chống bấm trùng tạo rác** qua `sig`. Đầu tab có expander **"📌 Ghi nhớ: kèo heuristic theo bội"** (`compute_heuristic_summary()`, ROI + bootstrap CI mỗi bội).
